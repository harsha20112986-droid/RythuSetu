...
function App() {
  ...
  const saveProfile = async (event: FormEvent) => {
    ...
    try {
      ...
      const saved: Farmer = { id: body.id, form };
      setFarmer(saved);
      ...
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 lg:px-8">
        ...
        <div className="flex items-center gap-2">
          {hasProfile && page !== "dashboard" && (
            <button onClick={onDashboard} className="hidden rounded-xl bg-green-50 px-4 py-2 text-sm font-semibold text-green-800 sm:block">
              My Dashboard
            </button>
          )}
          <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-800">Dashboard v0.8</span>
        </div>
      </div>
    </header>
  );
...
function Home({ hasProfile, onStart, onDashboard }: { hasProfile: boolean; onStart: () => void; onDashboard: () => void }) {
  const features = [
    ["🌦️", "Live Climate Risk", "Location-aware weather and an explainable risk signal."],
      ...
      <section className="mx-auto grid max-w-6xl gap-10 px-5 py-14 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-20">
        <div className="flex flex-col justify-center">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            ...
            {hasProfile ? (
              <button onClick={onStart} className="rounded-xl border border-stone-300 bg-white px-5 py-3 font-semibold">Update Profile</button>
            ) : (
              <a href="#features" className="rounded-xl border border-stone-300 bg-white px-5 py-3 font-semibold">Explore Features</a>
            )}
          </div>
          ...
          <div className="mt-6 rounded-2xl bg-white/10 p-5">
            ...
            <div className="mt-6 grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl bg-white/10 p-4">
                ...
                <p className="mt-1 text-xl font-bold">Live</p>
              </div>
              <div className="rounded-2xl bg-white/10 p-4">
  ...
  return (
    <section className="mx-auto max-w-3xl px-5 py-10 lg:px-8 lg:py-14">
      ...
      <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-9">
        ...
        <p className="mt-3 leading-7 text-stone-600">Your profile lets RythuSetu personalize climate, scheme, benefit, and crop-loss guidance.</p>
...
function Dashboard({ farmer, onEdit, onSchemes, onBenefits, onLoss }: { farmer: Farmer; onEdit: () => void; onSchemes: () => void; onBenefits: () => void; onLoss: () => void }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const response = await fetch(`${API_BASE}/climate/risk?${query}`, { signal: controller.signal });
        const body = await response.json().catch(() => null);
        if (!response.ok) throw new Error(body?.detail || "Unable to load climate data");
        setClimate(body as ClimateData);
      } catch (err) {
        if (err instanceof DOMException && err.name === "AbortError") return;
        setError(err instanceof Error ? err.message : "Unable to load climate data");
      } finally {
        setLoading(false);
      }
    };
  ...
  const risk = climate?.risk.level ?? "Loading";
  const riskClass = risk === "High" ? "bg-red-50 text-red-800" : risk === "Moderate" ? "bg-amber-50 text-amber-900" : "bg-green-50 text-green-800";
    ...
    <section className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
      <div className="flex flex-col gap-4 rounded-3xl bg-green-900 p-6 text-white shadow-xl sm:p-8 md:flex-row md:items-end md:justify-between">
        <div>
          ...
          <h1 className="mt-2 text-3xl font-black sm:text-4xl">Namaste, {farmer.form.name} 👋</h1>
          <p className="mt-2 text-green-100">{farmer.form.village}, {farmer.form.mandal}, {farmer.form.district} · {farmer.form.state}</p>
        </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <Card icon="🌱" label="Main crop" value={farmer.form.crop} detail={`${farmer.form.season} season`} />
        <Card icon="📐" label="Farm area" value={`${Number(farmer.form.land_area_acres).toFixed(1)} acres`} detail="Profile value" />
        <Card icon="📍" label="Location" value={farmer.form.district} detail={farmer.form.state} />
        <Card icon="🗣️" label="Language" value={farmer.form.language} detail="Preferred" />
      </div>
      ...
      <div className="mt-8 grid gap-5 lg:grid-cols-[1.35fr_0.85fr]">
        <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-7">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-sm font-semibold text-green-700">Live climate risk</p>
              <h2 className="mt-1 text-2xl font-black">{loading ? "Loading current conditions…" : error ? "Climate unavailable" : `${risk} attention`}</h2>
              <p className="mt-2 max-w-xl text-sm leading-6 text-stone-600">
                {error ? error : climate ? `Weather for ${climate.location.name}. The risk signal is an informational RythuSetu heuristic.` : "Fetching weather data…"}
              </p>
            </div>
            {!loading && !error && <span className={`rounded-full px-3 py-1 text-xs font-bold ${riskClass}`}>{risk}</span>}
          </div>

          {climate && !loading && !error && (
            <>
              <div className="mt-6 grid gap-4 sm:grid-cols-3">
                <Metric label="Temperature" value={`${climate.current.temperature_c}°C`} />
                <Metric label="Rain chance" value={`${climate.today_forecast.rain_probability_percent}%`} />
                <Metric label="Risk score" value={`${climate.risk.score}/10`} />
              </div>
              <div className="mt-4 grid gap-4 sm:grid-cols-3">
                <Metric label="Humidity" value={`${climate.current.humidity_percent}%`} />
                <Metric label="Rain today" value={`${climate.today_forecast.precipitation_sum_mm} mm`} />
                <Metric label="Wind gust" value={`${climate.today_forecast.max_wind_gust_kmh} km/h`} />
              </div>
              <div className="mt-5 rounded-2xl bg-stone-50 p-5">
                <p className="text-sm font-bold">Suggested action</p>
                <p className="mt-2 text-sm leading-6 text-stone-600">{climate.risk.suggested_action}</p>
                {climate.risk.factors.length > 0 && (
                  <div className="mt-3 flex flex-wrap gap-2">
                    {climate.risk.factors.map((factor) => <span key={factor} className="rounded-full bg-white px-3 py-1 text-xs font-medium text-stone-600 ring-1 ring-stone-200">{factor}</span>)}
                  </div>
                )}
              </div>
              <p className="mt-4 text-xs text-stone-400">Source: {climate.source}</p>
            </>
          )}
        </div>
        ...
        <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-7">
          ...
          <div className="mt-5 space-y-3">
            <button onClick={onSchemes} className="w-full text-left"><Action icon="📋" title="Find schemes" subtitle="Relevant government support" /></button>
            <button onClick={onBenefits} className="w-full text-left"><Action icon="💰" title="Estimate benefits" subtitle="Explainable eligibility estimate" /></button>
            <button onClick={onLoss} className="w-full text-left"><Action icon="🌱" title="Report crop loss" subtitle="Evidence + tracking workflow" /></button>
          </div>
        </div>
      </div>

      <div className="mt-6 rounded-2xl border border-green-100 bg-green-50 px-5 py-4 text-sm text-green-900">
        <strong>Profile ID #{farmer.id}.</strong> Your saved profile personalizes RythuSetu features.
      </div>
    </section>
  );
}

function SchemeFinder({ farmer, onBack }: { farmer: Farmer; onBack: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const query = new URLSearchParams({ state: farmer.form.state, crop: farmer.form.crop, season: farmer.form.season });
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
      ...
      <div className="rounded-3xl bg-green-900 p-6 text-white shadow-xl sm:p-8">
        ...
        <h1 className="mt-2 text-3xl font-black">Support matched to your farm</h1>
      ...
      {!loading && !error && (
        <div className="mt-8 grid gap-5 lg:grid-cols-2">
          {schemes.map((scheme) => (
            <article key={scheme.id} className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
              ...
              <div className="mt-4 rounded-2xl bg-stone-50 p-4"><p className="text-xs font-bold uppercase text-stone-500">Potential benefit</p><p className="mt-2 font-bold">{scheme.benefit}</p></div>
              <p className="mt-4 text-sm font-bold">Why it appeared</p>
              <ul className="mt-2 space-y-1 text-sm text-stone-600">{scheme.reasons.map((reason) => <li key={reason}>✓ {reason}</li>)}</ul>
              <div className="mt-4 rounded-2xl border border-stone-200 p-4 text-sm"><b>Eligibility note</b><p className="mt-2 leading-6 text-stone-600">{scheme.eligibility_note}</p></div>
              <div className="mt-5 flex items-center justify-between gap-3"><span className="text-xs text-stone-400">Source checked: {scheme.last_verified}</span><a href={scheme.official_url} target="_blank" rel="noreferrer" className="rounded-xl bg-green-700 px-4 py-2 text-sm font-bold text-white">Open official site ↗</a></div>
            </article>
          ))}
        </div>
      )}
...
function BenefitEstimator({ farmer, onBack }: { farmer: Farmer; onBack: () => void }) {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const query = new URLSearchParams({ state: farmer.form.state, crop: farmer.form.crop, season: farmer.form.season, land_area_acres: farmer.form.land_area_acres });
        const response = await fetch(`${API_BASE}/benefits/estimate?${query}`);
        const body = await response.json();
        if (!response.ok) throw new Error(body?.detail || "Unable to estimate benefits");
        setData(body as BenefitData);
      } catch (err) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
      ...
      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-6 text-amber-900"><b>Important:</b> This is an informational estimate, not a guaranteed payment or insurance claim. Official eligibility, records, exclusions, enrollment, verification and assessed losses determine actual outcomes.</div>
      {loading && <Loading text="Calculating potential support…" />}
      {error && <ErrorBox text={error} />}
      {data && !loading && !error && (
        <>
          <div className="mt-5 rounded-3xl border border-green-100 bg-green-50 p-6"><p className="text-sm text-green-800">Potential annual support covered by fixed rules</p><p className="mt-1 text-4xl font-black text-green-800">₹{data.estimated_total.toLocaleString("en-IN")}</p><p className="mt-2 text-xs text-green-900">Only schemes with a responsible fixed or area-based calculation are included.</p></div>
          <div className="mt-6 grid gap-5 lg:grid-cols-2">
            {data.items.map((item) => (
              <article key={item.id} className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
                ...
                <div className="mt-5 rounded-2xl bg-stone-50 p-4"><p className="text-xs font-bold uppercase text-stone-500">Calculation</p><p className="mt-2 font-bold">{item.calculation}</p></div>
                <p className="mt-4 text-sm leading-6 text-stone-600"><b>Basis:</b> {item.basis}</p>
                <div className="mt-5 flex items-center justify-between"><span className="text-xs text-stone-400">Source checked: {item.last_verified}</span><a href={item.official_url} target="_blank" rel="noreferrer" className="rounded-xl bg-green-700 px-4 py-2 text-sm font-bold text-white">Verify officially ↗</a></div>
              </article>
            ))}
          </div>
          <div className="mt-5 rounded-2xl border border-stone-200 bg-white p-5 text-sm leading-6 text-stone-600"><b>What can change the actual amount?</b><p className="mt-1">Government rules, land-record verification, family-level eligibility, scheme exclusions, enrollment status, notified areas/crops, and insurance loss assessment can change the final outcome.</p></div>
        </>
      )}
    </section>
  );
...
function CropLossReporter({ farmer, onBack }: { farmer: Farmer; onBack: () => void }) {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
      ...
      <div className="mt-5 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4 text-sm leading-6 text-amber-900"><b>Important:</b> Your damage percentage is a farmer report, not an official loss assessment.</div>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_0.8fr]">
        <form onSubmit={submit} className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
          ...
          <div className="mt-5 grid gap-5 sm:grid-cols-2">
            <Field label="Crop" value={farmer.form.crop} onChange={() => undefined} readOnly />
          ...
          <button disabled={submitting} className="mt-6 w-full rounded-xl bg-green-700 px-5 py-3.5 font-bold text-white hover:bg-green-800 disabled:opacity-60">{submitting ? "Submitting…" : "Submit Loss Report"}</button>
        </form>

        <div>
          <h2 className="text-xl font-black">My reports</h2>
          {loading && <Loading text="Loading reports…" />}
          {!loading && reports.length === 0 && <div className="mt-4 rounded-2xl border border-stone-200 bg-white p-6 text-sm text-stone-600">No crop-loss reports yet.</div>}
          {!loading && reports.map((report) => (
            <article key={report.id} className="mt-4 rounded-3xl border border-stone-200 bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3">
                <div><p className="text-xs font-semibold text-green-700">Report #{report.id}</p><h3 className="mt-1 font-black">{report.crop} · {report.damage_type}</h3></div>
              ...
              <div className="mt-4 rounded-2xl border border-stone-200 p-4 text-sm"><b>Next step</b><p className="mt-1 leading-6 text-stone-600">{report.next_step}</p></div>
            </article>
          ))}
        </div>
      </div>
...
function Metric({ label, value }: { label: string; value: string }) {
  return <div className="rounded-2xl border border-stone-200 p-4"><p className="text-xs font-semibold uppercase tracking-wide text-stone-500">{label}</p><p className="mt-2 text-2xl font-black">{value}</p></div>;
}
...
function Field({ label, value, onChange, placeholder, type = "text", min, step, required = false, readOnly = false }: { label: string; value: string; onChange: (value: string) => void; placeholder?: string; type?: string; min?: string; step?: string; required?: boolean; readOnly?: boolean }) {
  return <label className="block"><span className="mb-2 block text-sm font-semibold text-stone-700">{label}</span><input required={required} readOnly={readOnly} type={type} min={min} step={step} value={value} onChange={(event) => onChange(event.target.value)} placeholder={placeholder} className="w-full rounded-xl border border-stone-300 bg-white px-4 py-3 outline-none focus:border-green-600 focus:ring-4 focus:ring-green-100 read-only:bg-stone-50" /></label>;
}
