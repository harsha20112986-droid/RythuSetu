...
function App() {
  const [page, setPage] = useState<"home" | "onboarding" | "dashboard">("home");
  ...
  return (
    <main className="min-h-screen bg-stone-50 text-stone-900">
      ...
      {page === "home" && (
        <Home hasProfile={Boolean(farmer)} onStart={() => setPage("onboarding")} onDashboard={() => setPage("dashboard")} />
      )}

      {page === "onboarding" && (
        <section className="mx-auto max-w-3xl px-5 py-10 lg:px-8 lg:py-14">
          <button onClick={() => setPage(farmer ? "dashboard" : "home")} className="mb-6 text-sm font-semibold text-green-700">← Back</button>
            ...
            <form onSubmit={saveProfile} className="mt-8 space-y-6">
              ...
              <div className="border-t border-stone-100 pt-6">
                ...
                <div className="mt-4 grid gap-5 sm:grid-cols-2">
                  <SelectField label="State" value={form.state} options={states} onChange={(v) => { update("state", v); update("district", districts[v][0]); }} />
      ...
      {page === "dashboard" && farmer && <Dashboard farmer={farmer} onEdit={() => setPage("onboarding")} />}
    ...
    <header className="border-b border-stone-200 bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between px-5 py-4 lg:px-8">
        <button onClick={onHome} className="flex items-center gap-3 text-left">
          ...
          <div><p className="text-lg font-bold">RythuSetu</p><p className="text-xs text-stone-500">Your AI Bridge to Farmer Support</p></div>
        </button>
        <div className="flex items-center gap-2">
          {hasProfile && page !== "dashboard" && <button onClick={onDashboard} className="hidden rounded-xl bg-green-50 px-4 py-2 text-sm font-semibold text-green-800 sm:block">My Dashboard</button>}
          <span className="rounded-full bg-green-50 px-3 py-1 text-xs font-semibold text-green-800">Dashboard v0.3</span>
        </div>
      </div>
    </header>
  );
  ...
  return <>
    <section className="mx-auto grid max-w-6xl gap-10 px-5 py-14 lg:grid-cols-[1.2fr_0.8fr] lg:px-8 lg:py-20">
      ...
      <div className="rounded-3xl bg-green-900 p-6 text-white shadow-xl sm:p-8">
        ...
        <div className="mt-6 rounded-2xl bg-white/10 p-5"><p className="text-sm text-green-100">Good morning 👋</p><p className="mt-1 text-2xl font-bold">Your farm support snapshot</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2"><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">Weather risk</p><p className="mt-1 text-xl font-bold">On dashboard</p></div><div className="rounded-2xl bg-white/10 p-4"><p className="text-xs text-green-100">Farmer profile</p><p className="mt-1 text-xl font-bold">{hasProfile ? "Saved" : "Not started"}</p></div></div>
        </div>
      </div>
    </section>
...
function Dashboard({ farmer, onEdit }: { farmer: SavedFarmer; onEdit: () => void }) {
  ...
  return <section className="mx-auto max-w-6xl px-5 py-8 lg:px-8 lg:py-12">
    ...
    <div className="mt-8 grid gap-5 lg:grid-cols-[1.4fr_0.8fr]">
      ...
      <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm sm:p-7"><p className="text-sm font-semibold text-green-700">Next actions</p><h2 className="mt-1 text-2xl font-black">Your support toolkit</h2><div className="mt-5 space-y-3"><Action icon="📋" title="Find schemes" subtitle="Relevant government support" /><Action icon="💰" title="Estimate benefits" subtitle="Explainable eligibility estimate" /><Action icon="🌱" title="Report crop loss" subtitle="Evidence + tracking workflow" /></div></div>
    </div>
